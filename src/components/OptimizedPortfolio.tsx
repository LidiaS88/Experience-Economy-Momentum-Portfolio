...
export const OptimizedPortfolio: React.FC<OptimizedPortfolioProps> = ({ dataMap }) => {
  ...
  // 2. Execute Deterministic Minimum-Variance Optimization
  const optimizationResult: OptimizationResult = useMemo(() => {
    return runPortfolioOptimizer(screeningItems, dataMap);
  ...
  const filteredHoldings = useMemo(() => {
    return optimizationResult.holdings.filter((h) => {
      ...
      return matchSearch && matchCategory && matchInclusion;
    });
  ...
  return (
    <section id="optimized-portfolio-section" className="mb-8">
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <div className="p-5 border-b border-slate-200 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 bg-slate-50/75">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{showMathDetails ? 'Hide Optimizer Math' : 'Inspect Optimizer Math & Proof'}</span>
            </button>
          </div>
        </div>
        ...
        {/* Fallback Active Prominent Banner (Requirement 2) */}
        ...
        {/* Validation Failure Warning (Requirement 7) */}
        {optimizationResult.status === 'Validation Fallback' && (
          <div
        ...
        {/* Insufficient Holdings Error Panel (Requirement 3) */}
          ...
          <div id="insufficient-data-stop-panel" className="p-8 sm:p-12 text-center bg-slate-50/50">
            <div className="max-w-md mx-auto flex flex-col items-center justify-center">
              ...
              <p className="text-[11px] text-slate-400 mt-4">
                Please enter your Twelve Data API key in the Data Access Panel above and click "Load Portfolio History" to populate real data.
              </p>
            </div>
          </div>
        ) : (
          <>
            {/* Optimizer Validation Panel (Requirement: Validation Panel) */}
            ...
            {/* Holdings Table (Requirement: Optimized Portfolio Table) */}
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-200">
                  {filteredHoldings.length > 0 ? (
                    filteredHoldings.map((h) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* 8. Latest Price */}
                          <td className="py-3 px-4 text-right font-mono whitespace-nowrap text-slate-800">
                            {formatPrice(h.latestPrice)}
                          </td>
                    ...
                    <tr>
                      <td colSpan={9} className="py-8 px-4 text-center text-slate-500 text-xs bg-slate-50/50">
                ...
                <tfoot>
                  <tr className="bg-slate-100 font-bold border-t-2 border-slate-300 text-slate-900 text-xs">
                    ...
                    <td colSpan={2} className="py-3 px-4 text-center text-slate-500 font-normal text-[11px]">
